...
class TrainingStatus:
  ...
  def toc(self, iter, train_loss):
    ...
    msg = ("%.2f%% (%d/%d): Train loss: %.3e t %.3f  @ %s (%s)" % (iter * 100.0 / self.num_steps, iter, self.num_steps, train_loss, self.end - self.start, time.strftime("%a %d %H:%M:%S", time.localtime(time.time() + etaTime)), etaString))
