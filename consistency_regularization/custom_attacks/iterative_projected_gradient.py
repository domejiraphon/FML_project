...
from custom_attacks.attack_utils import clamp
from custom_attacks.attack_utils import normalize_by_pnorm
from custom_attacks.attack_utils import clamp_by_pnorm
from custom_attacks.attack_utils import is_float_or_torch_tensor
from custom_attacks.attack_utils import batch_multiply
from custom_attacks.attack_utils import batch_clamp
from custom_attacks.attack_utils import replicate_input
from custom_attacks.attack_utils import batch_l1_proj

from custom_attacks.base import Attack
from custom_attacks.base import LabelMixin
from custom_attacks.utils import rand_init_delta
EPS = 1E-20
...
def custom_perturb_iterative(xvar, yvar, predict, nb_iter, eps, 
                      eps_iter, loss_fn, proxy, proxy_optim, 
                      delta_init=None, minimize=False, ord=np.inf,
                      clip_min=0.0, clip_max=1.0,
                      l1_sparsity=None):
    """
    ...
    delta.requires_grad_()
    proxy.load_state_dict(predict.state_dict())
    proxy.train()
    for ii in range(nb_iter):
        outputs = proxy(xvar + delta)
        loss = loss_fn(outputs, yvar)
        if minimize:
            loss = -loss
    ...
    x_adv = clamp(xvar + delta, clip_min, clip_max)
    diff = diff_in_weights(predict, proxy)
    return x_adv, diff

class CustomAttack(Attack, LabelMixin):
    """
    The projected gradient descent attack (Madry et al, 2017).
    The attack performs nb_iter steps of size eps_iter, while always staying
    within eps from the initial point.
    ...
    def __init__(
        ...
        self.l1_sparsity = l1_sparsity
        self.gamma = 1e-2
        self.proxy = copy.deepcopy(self.predict).half().to("cuda")
        self.proxy_optim = torch.optim.SGD(self.proxy.parameters(), 
                              lr=self.gamma/self.nb_iter)
    ...
    def perturb(self, x, y=None):
        ...
        rval, diff = custom_perturb_iterative(
            x, y, self.predict, nb_iter=self.nb_iter,
            eps=self.eps, eps_iter=self.eps_iter,
            loss_fn=self.loss_fn, minimize=self.targeted,
            ord=self.ord, clip_min=self.clip_min,
            clip_max=self.clip_max, delta_init=delta,
            l1_sparsity=self.l1_sparsity, proxy=self.proxy,
            proxy_optim=self.proxy_optim,
        )

        return rval.data, diff
